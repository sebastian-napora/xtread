...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env['XTREAD_SANDBOX']) return 30000; // 30s in containers
  ...
  setup(
    ...
    const settings = {
      ...
      sandbox: env.XTREAD_SANDBOX !== 'false' ? env.XTREAD_SANDBOX : false,
  ...
  /**
   * The command and args to use to invoke Xtread Code CLI. Allows us to switch
    ...
    return this.poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `xtread-code.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'xtread-code.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'xtread-code.tool_call'
      ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'xtread-code.api_request',
    );
      ...
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `xtread-code.${metricName}`) {
              return metric;
            }
          }
        }
