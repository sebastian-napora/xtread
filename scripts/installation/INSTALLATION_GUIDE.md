# Installation Guide for Xtread Code with Source Tracking

This guide describes how to install Node.js and Xtread Code with source information tracking.

## Overview

The installation scripts automate the process of installing Node.js (if not present or below version 20) and Xtread Code, while capturing and storing the installation source information for analytics and tracking purposes.

## Installation Scripts

We provide platform-specific installation scripts:

- **Linux/macOS**: `install-xtread-with-source.sh`
- **Windows**: `install-xtread-with-source.bat`

## Linux/macOS Installation

### Script: install-xtread-with-source.sh

#### Features:

- Checks for existing Node.js installation and version
- Installs Node.js 20+ if needed using NVM
- Installs Xtread Code globally with source information
- Stores the source information in `~/.xtread/source.json`

#### Usage:

```bash
# Install with a specific source
sh install-xtread-with-source.sh --source github

# Install with internal source
sh install-xtread-with-source.sh -s internal

# Show help
sh install-xtread-with-source.sh --help
```
...
1. The script accepts a `--source` parameter to specify where Xtread Code is being installed from
2. It installs Node.js if needed
3. It installs Xtread Code globally
4. It creates `~/.xtread/source.json` with the specified source information
...
This is required to load the newly installed Node.js and Xtread Code into your PATH.
...
### Script: install-xtread-with-source.bat
...
- Internet connection for downloading Node.js and Xtread Code
...
./install-xtread-with-source.bat --source github

# Install with short parameter
./install-xtread-with-source.bat -s internal

# Use default source (unknown)
./install-xtread-with-source.bat
```
...
1. The script accepts a `--source` or `-s` parameter to specify where Xtread Code is being installed from
...
4. It installs Xtread Code globally using npm
...
This feature implements the ability to capture and store the installation source of the Xtread Code package. The source information is used for analytics and tracking purposes.

### Storage Location

The installation source is stored in a separate file at:

- **Unix/Linux/macOS**: `~/.xtread/source.json`
...
2. **Analytics**: Helps understand how users are discovering and installing Xtread Code
...
./install-xtread-with-source.bat -s github
```
...
- Ensure NVM is installed: `curl -o- https://xtread-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh | bash`
...
- The scripts require internet access to download Node.js and Xtread Code
