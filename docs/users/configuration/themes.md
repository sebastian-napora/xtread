...
Xtread Code supports a variety of themes to customize its color scheme and appearance. You can change the theme to suit your preferences via the `/theme` command or `"theme":` configuration setting.

## Available Themes

Xtread Code comes with a selection of pre-defined themes, which you can list using the `/theme` command within the CLI:
...
1.  Enter `/theme` into Xtread Code.
...
Selected themes are saved in Xtread Code's [configuration](../configuration/settings) so your preference is remembered across sessions.

---

## Custom Color Themes

Xtread Code allows you to create your own custom color themes by specifying them in your `settings.json` file. This gives you full control over the color palette used in the CLI.
...
- Select your custom theme using the `/theme` command in Xtread Code. Your custom theme will appear in the theme selection dialog.
