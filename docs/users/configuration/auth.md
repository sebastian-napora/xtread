...
Xtread Code supports three authentication methods. Pick the one that matches how you want to run the CLI:
...
- **How it works**: on first start, Xtread Code opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure Xtread Code to use the dedicated endpoint and your subscription API key.
...
**Option B: Inside a Xtread Code session**

Enter `qwen` in the terminal to launch Xtread Code, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.
...
If you prefer to skip the interactive `/auth` flow, add the following to `~/.xtread/settings.json`:
...
The simplest way to get started with API Key authentication is to put everything in a single `~/.xtread/settings.json` file. Here's a complete, ready-to-use example:
...
| `security.auth.selectedType` | Tells Xtread Code which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when Xtread Code starts. Must match one of the `id` values in your `modelProviders`.                                |
...
The key concept is **Model Providers** (`modelProviders`): Xtread Code supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.xtread/settings.json`, then switch between them at runtime with the `/model` command.
...
#### Step 1: Configure models and providers in `~/.xtread/settings.json`
...
> [!important]
>
> It is recommended to define `modelProviders` in the user-scope `~/.xtread/settings.json` to avoid merge conflicts between project and user settings.

Edit `~/.xtread/settings.json` (create it if it doesn't exist). You can mix multiple protocols in a single file — here is a multi-provider example showing just the `modelProviders` section:
...
Xtread Code reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
Xtread Code auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.xtread/.env` (preferred — keeps Xtread Code variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.xtread/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with Xtread Code behavior.

**3. `settings.json` → `env` field (lowest priority)**

You can also define API keys directly in `~/.xtread/settings.json` under the `env` key. These are loaded as the **lowest-priority fallback** — only applied when a variable is not already set by the system environment or `.env` files.
...
After launching Xtread Code, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
In addition to the in-session `/auth` slash command, Xtread Code provides a standalone `qwen auth` CLI command for managing authentication directly from the terminal — without starting an interactive session first.
...
- Prefer `.xtread/.env` for project-local secrets (and keep it out of git).
