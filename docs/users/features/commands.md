...
This document details all commands supported by Xtread Code, helping you efficiently manage sessions, customize the interface, and control its behavior.

Xtread Code commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of Xtread Code itself              | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage Xtread Code sessions, interface, and basic behavior.
...
| `/qc-helper` | Answer questions about Xtread Code usage and configuration            | `/qc-helper how do I configure MCP?`              |
...
| `/bug`      | Submit issue about Xtread Code                    | `/bug Button click unresponsive` |
...
In addition to the in-session `/auth` slash command, Xtread Code provides standalone CLI subcommands for managing authentication directly from the terminal:
...
> [!tip]
>
> These commands run outside of a Xtread Code session. Use them to configure authentication before starting a session, or in scripts and CI environments. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within Xtread Code.
...
| Global Commands  | `~/.xtread/commands/`                        | Available in all projects              | Low      | Personal frequently used commands, cross-project use |
| Project Commands | `<project root directory>/.xtread/commands/` | Project-specific, version-controllable | High     | Team sharing, project-specific commands              |
...
| ---------------------------------------- | ----------------- | --------------------- |
| `~/.xtread/commands/test.md`               | `/test`           | `/test Parameter`     |
| `<project>/.xtread/commands/git/commit.md` | `/git:commit`     | `/git:commit Message` |
...
| ----------------------------- | ----------------------------------------- |
| 1. Create directory structure | `mkdir -p ~/.xtread/commands/refactor`      |
| 2. Create command file        | `touch ~/.xtread/commands/refactor/pure.md` |
