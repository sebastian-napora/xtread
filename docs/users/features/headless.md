...
Headless mode allows you to run Xtread Code programmatically from command line
scripts and automation tools without any interactive UI. This is ideal for
scripting, automation, CI/CD pipelines, and building AI-powered tools.

## Overview

The headless mode provides a headless interface to Xtread Code that:
...
Pipe input to Xtread Code from your terminal:
...
Read from files and process with Xtread Code:
...
> [!note]
>
> - Session data is project-scoped JSONL under `~/.xtread/projects/<sanitized-cwd>/chats`.
...
Use `--system-prompt` to replace Xtread Code's built-in main-session prompt for the current run:
...
Xtread Code supports multiple output formats for different use cases:
...
The `--input-format` parameter controls how Xtread Code consumes input from standard input:
