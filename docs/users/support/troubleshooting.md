...
- **Error: `Device authorization flow failed: fetch failed`**
  - **Cause:** Node.js could not reach Qwen OAuth endpoints (often a proxy or SSL/TLS trust issue). When available, Xtread Code will also print the underlying error cause (for example: `UNABLE_TO_VERIFY_LEAF_SIGNATURE`). Note: this error is specific to the legacy Qwen OAuth flow.
  - **Solution:**
...
- **Issue: Unable to display UI after authentication failure**
  ...
  - **Solution:** Clear the `security.auth.selectedType` configuration item in your `settings.json` file:
    - Open `~/.xtread/settings.json` (or `./.xtread/settings.json` for project-specific settings)
    - Remove the `security.auth.selectedType` field
    - Restart the CLI to allow it to prompt for authentication again

## Frequently asked questions (FAQs)

- **Q: How do I update Xtread Code to the latest version?**
  - A: If you installed it globally via `npm`, update it using the command `npm install -g @xtread-code/xtread-code@latest`. If you compiled it from source, pull the latest changes from the repository, and then rebuild using the command `npm run build`.

- **Q: Where are the Xtread Code configuration or settings files stored?**
  - A: The Xtread Code configuration is stored in two `settings.json` files:
    1. In your home directory: `~/.xtread/settings.json`.
    2. In your project's root directory: `./.xtread/settings.json`.

    Refer to [Xtread Code Configuration](../configuration/settings) for more details.
...
- **Error: Command not found (when attempting to run Xtread Code with `qwen`).**
  - **Cause:** The CLI is not correctly installed or it is not in your system's `PATH`.
  - **Solution:**
    The update depends on how you installed Xtread Code:
    - If you installed `qwen` globally, check that your `npm` global binary directory is in your `PATH`. You can update using the command `npm install -g @xtread-code/xtread-code@latest`.
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Xtread Code may attempt operations that are restricted by your sandbox configuration, such as writing outside the project directory or system temp directory.
...
- **Xtread Code is not running in interactive mode in "CI" environments**
  - **Issue:** Xtread Code does not enter interactive mode (no prompt appears) if an environment variable starting with `CI_` (e.g. `CI_TOKEN`) is set. This is because the `is-in-ci` package, used by the underlying UI framework, detects these variables and assumes a non-interactive CI environment.
...
- **DEBUG mode not working from project .env file**
  ...
  - **Solution:** Use a `.xtread/.env` file instead, or configure the `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer variables.
...
- Reinstall the companion with `/ide install` and use “Xtread Code: Run” in the Command Palette to verify it launches.

## Exit Codes

The Xtread Code uses specific exit codes to indicate the reason for termination. This is especially useful for scripting and automation.
...
If you encounter an issue that was not covered here in this _Troubleshooting guide_, consider searching the Xtread Code [Issue tracker on GitHub](https://github.com/QwenLM/xtread-code/issues). If you can't find an issue similar to yours, consider creating a new GitHub Issue with a detailed description. Pull requests are also welcome!
