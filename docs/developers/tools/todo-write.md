...
This document describes the `todo_write` tool for Xtread Code.
...
## How to use `todo_write` with Xtread Code
...
The tool stores todo lists in your home directory (`~/.xtread/todos/`) with session-specific files, so each coding session maintains its own task list.
