# Xtread Code file system tools

Xtread Code provides a comprehensive suite of tools for interacting with the local file system. These tools allow the model to read from, write to, list, search, and modify files and directories, all under your control and typically with confirmation for sensitive operations.
...
- **Behavior:**
  ...
  - Respects .gitignore and .xtreadignore patterns by default.
...
- **Behavior:**
  ...
  - Respects .gitignore and .xtreadignore patterns.
...
When reading files, Xtread Code detects the file's encoding using a multi-step strategy:
...
Set it in `.xtread/settings.json` or `~/.xtread/settings.json`:
...
These file system tools provide a foundation for Xtread Code to understand and interact with your local project context.
