...
This document describes the `task` tool for Xtread Code.
...
## How to use `task` with Xtread Code
...
You can view available subagents by using the `/agents` command in Xtread Code.
...
Subagents are configured through Xtread Code's agent configuration system. Use the `/agents` command to:
