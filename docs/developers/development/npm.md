...
This monorepo contains two main packages: `@xtread-code/xtread-code` and `@xtread-code/xtread-code-core`.

## `@xtread-code/xtread-code`

This is the main package for Xtread Code. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@xtread-code/xtread-code-core`. This means that whether a user installs the package with `npm install -g @xtread-code/xtread-code` or runs it directly with `npx @xtread-code/xtread-code`, they are using this single, self-contained executable.

## `@xtread-code/xtread-code-core`
...
Releases are managed through the [release.yml](https://github.com/QwenLM/xtread-code/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @xtread-code/xtread-code@preview

# Nightly
npm install -g @xtread-code/xtread-code@nightly
```
...
- `npx -y @xtread-code/xtread-code@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @xtread-code/xtread-code@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @xtread-code/xtread-code && npm uninstall -g @xtread-code/xtread-code && npm cache clean --force &&  npm install @xtread-code/xtread-code@<version>`
...
1.  Go to the [Actions tab](https://github.com/QwenLM/xtread-code/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/xtread-code-0.1.6.tgz`).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @xtread-code/xtread-code`.
