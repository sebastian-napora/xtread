# @xtread-code/channel-base

Base infrastructure for building Xtread Code channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @xtread-code/channel-base
```
...
For a complete working example, see [`@xtread-code/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `xtread-code --acp` agent process |
...
Manages the `xtread-code --acp` child process and ACP sessions.
...
Persists pairing state to `~/.xtread/channels/{channelName}-pairing.json` and `{channelName}-allowlist.json`.
...
- [`@xtread-code/channel-plugin-example`](../plugin-example/) — working reference implementation
