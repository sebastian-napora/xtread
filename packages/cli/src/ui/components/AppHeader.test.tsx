...
const createMockConfig = (overrides = {}) => ({
  ...
  getTargetDir: vi.fn(() => '/projects/xtread-code'),
  getMcpServers: vi.fn(() => ({})),
...
describe('<AppHeader />', () => {
  it('shows the working directory', () => {
    ...
    expect(lastFrame()).toContain('/projects/xtread-code');
  });
  ...
  it('hides the header when screen reader is enabled', () => {
    ...
    expect(lastFrame()).not.toContain('/projects/xtread-code');
    expect(lastFrame()).not.toContain('Xtread Code');
  });

  it('shows the header with all info when banner is visible', () => {
    const { lastFrame } = renderWithProviders(createMockUIState());
    expect(lastFrame()).toContain('>_ XtreaD');
