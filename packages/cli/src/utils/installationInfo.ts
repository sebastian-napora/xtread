...
import { createDebugLogger, isGitRepository } from '@xtread-code/xtread-core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^xtread-code$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @xtread-code/xtread@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @xtread-code/xtread@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @xtread-code/xtread@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @xtread-code/xtread@latest';
    return {
