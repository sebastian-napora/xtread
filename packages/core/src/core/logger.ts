...
export class Logger {
  private xtreadDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.xtreadDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.xtreadDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.xtreadDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.xtreadDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.xtreadDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.xtreadDir) {
      this.debugLogger.error(
