...
export function getCoreSystemPrompt(
  ...
  // default path is .xtread/system.md but can be modified via custom path in QWEN_SYSTEM_MD
  ...
  const basePrompt = systemMdEnabled
    ...
    : `
You are Xtread Code, an interactive CLI agent developed by Alibaba Group, specializing in software engineering tasks. Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.
...
function getToolCallExamples(model?: string): string {
  ...
  if (model && model.length < 100) {
    // Match qwen*-coder patterns (e.g., qwen3-coder, qwen2.5-coder, xtread-coder)
...
const INSIGHT_PROMPTS: Record<InsightPromptType, string> = {
  analysis: `Analyze this Xtread Code session and extract structured facets.
...
4. If very short or just warmup, use warmup_minimal for goal_category`,
  ...
  impressive_workflows: `Analyze this Xtread Code usage data and identify what's working well for this user. Use second person ("you").
...
Include 3 impressive workflows.`,
  ...
  project_areas: `Analyze this Xtread Code usage data and identify project areas.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "areas": [
    {"name": "Area name", "session_count": N, "description": "2-3 sentences about what was worked on and how Xtread Code was used."}
  ]
}

Include 4-5 areas. Skip internal QC operations.`,

  future_opportunities: `Analyze this Xtread Code usage data and identify future opportunities.
...
Include 3 opportunities. Think BIG - autonomous workflows, parallel agents, iterating against tests.`,
  ...
  friction_points: `Analyze this Xtread Code usage data and identify friction points for this user. Use second person ("you").
...
Include 3 friction categories with 2 examples each.`,
  ...
  memorable_moment: `Analyze this Xtread Code usage data and find a memorable moment.
...
Find something genuinely interesting or amusing from the session summaries.`,
  ...
  improvements: `Analyze this Xtread Code usage data and suggest improvements.
...
2. **Custom Skills**: Reusable prompts you define as markdown files that run with a single /command.
   - How to use: Create \`.xtread/skills/commit/SKILL.md\` with instructions. Then type \`/commit\` to run it.
...
IMPORTANT for features_to_try: Pick 2-3 from the QC FEATURES REFERENCE above. Include 2-3 items for each category.`,
  ...
  interaction_style: `Analyze this Xtread Code usage data and describe the user's interaction style.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "narrative": "2-3 paragraphs analyzing HOW the user interacts with Xtread Code. Use second person 'you'. Describe patterns: iterate quickly vs detailed upfront specs? Interrupt often or let Qwen run? Include specific examples. Use **bold** for key insights.",
  "key_pattern": "One sentence summary of most distinctive interaction style"
}
`,

  at_a_glance: `You're writing an "At a Glance" summary for a Xtread Code usage insights report for Xtread Code users. The goal is to help them understand their usage and improve how they can use Qwen better, especially as models improve.
...
3. **Quick wins to try** - Specific Xtread Code features they could try from the examples below, or a workflow technique if you think it's really compelling. (Avoid stuff like "Ask Qwen to confirm before taking actions" or "Type out more context up front" which are less compelling.)
