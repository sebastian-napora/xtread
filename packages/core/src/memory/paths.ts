 ...
 * Defaults to `~/.qwen`; overridable via XTREAD_CODE_MEMORY_BASE_DIR for tests.
 */
export function getMemoryBaseDir(): string {
  if (process.env['XTREAD_CODE_MEMORY_BASE_DIR']) {
    return process.env['XTREAD_CODE_MEMORY_BASE_DIR'];
  }
...
export function getAutoMemoryRoot(projectRoot: string): string {
  ...
  let result: string;
  if (process.env['XTREAD_CODE_MEMORY_LOCAL'] === '1') {
