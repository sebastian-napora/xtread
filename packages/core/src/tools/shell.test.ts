...
describe('ShellTool', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      storage: {
        getUserSkillsDirs: vi.fn().mockReturnValue(['/test/dir/.xtread/skills']),
  ...
  describe('build', () => {
    ...
    it('should throw an error for a directory within the user skills directory', async () => {
      expect(() =>
        shellTool.build({
          command: 'ls',
          directory: '/test/dir/.xtread/skills/my-skill',
          is_background: false,
        }),
      ).toThrow(
    ...
    it('should throw an error for the user skills directory itself', async () => {
      expect(() =>
        shellTool.build({
          command: 'ls',
          directory: '/test/dir/.xtread/skills',
          is_background: false,
        }),
      ).toThrow(
        'Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.',
    ...
    it('should resolve directory path before checking user skills directory', async () => {
      expect(() =>
        shellTool.build({
          command: 'ls',
          directory: '/test/dir/.xtread/skills/../skills/my-skill',
          is_background: false,
        }),
      ).toThrow(
        'Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.',
      ...
      it('should add co-author to git commit with double quotes', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(
          expect.stringContaining(
            'Co-authored-by: xtread-coder <xtread-coder@alibabacloud.com>',
          ),
          expect.any(String),
          expect.any(Function),
          expect.any(AbortSignal),
