...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .xtreadignore to ripgrep when respected', async () => {
      await fs.writeFile(
        path.join(tempRootDir, '.xtreadignore'),
        'ignored.txt\n',
      );
      (runRipgrep as Mock).mockResolvedValue({
        stdout: '',
    ...
    it('should include .xtreadignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.xtreadignore'), 'kept.txt\n');
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .xtreadignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.xtreadignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.xtreadignore'),
        'other-ignored.txt\n',
      );
      ...
      // Verify both .xtreadignore files were passed
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.xtreadignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.xtreadignore'));
