...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'xtread-code-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `xtread-code-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `xtread-code-modify-${fileName}-new-${timestamp}${ext}`,
  );
