...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .xtreadignore patterns even when not in a git repo', async () => {
      await createTestFile('.xtreadignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldXtreadIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldXtreadIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.xtreadignore', 'logs/');
    });
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectXtreadIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectXtreadIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldXtreadIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.xtreadignore', '*.log');
    });
    ...
    it('should return true for qwen-ignored files', () => {
      ...
      expect(
        service.shouldXtreadIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });
    ...
    it('should return false for non-qwen-ignored files', () => {
      ...
      expect(
        service.shouldXtreadIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
