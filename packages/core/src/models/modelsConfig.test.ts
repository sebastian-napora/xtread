...
describe('ModelsConfig', () => {
  ...
  it('should maintain consistency between currentModelId and _generationConfig.model after initialization', () => {
    ...
    expect(config3.getModel()).toBe('coder-model'); // Falls back to DEFAULT_XTREAD_MODEL
  ...
  describe('getAllConfiguredModels', () => {
    it('should return all models across all authTypes and put xtread-oauth first', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
    ...
    it('should return empty array when no models are registered', () => {
      ...
      // Should be empty when no models are registered
    ...
    it('should support filtering by authTypes and still put xtread-oauth first when included', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // Filter: OpenAI only (should not include xtread-oauth)
    ...
    it('should handle reload with empty config', async () => {
      ...
      expect(
        modelsConfig
          ...
          .filter((m) => m.authType === AuthType.USE_OPENAI).length,
      ).toBeGreaterThan(0);

      // Reload with empty config
      modelsConfig.reloadModelProvidersConfig({});

      // No models should remain after empty reload
