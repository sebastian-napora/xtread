...
`.trim();
  ...
  describe('with xtreadignore', () => {
    it('should ignore xtreadignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.xtreadignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectXtreadIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.xtreadignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
