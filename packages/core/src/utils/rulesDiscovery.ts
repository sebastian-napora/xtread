...
//
// Discovers .xtread/rules/ files (recursively) with optional YAML frontmatter.
...
/**
 * Discover and load rule files from a single `.xtread/rules/` directory.
...
export function formatRules(rules: RuleFile[], projectRoot: string): string {
  return rules
    .map((rule) => {
      ...
      // and Linux shows `.xtread/rules/foo.md`, which is confusing in diffs/tests.
...
/**
 * Load rules from both global (`~/.xtread/rules/`) and project-level
 * (`.xtread/rules/`) directories.
 *
...
export async function loadRules(
  ...
  // 2. Project-level rules: <projectRoot>/.xtread/rules/  (trusted only)
